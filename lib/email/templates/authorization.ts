...
`;
  ...
  const bookingDetailsHtml =
    ...
    Array.isArray(authForm.bookingDetails) &&
    authForm.bookingDetails.length > 0
      ? `
<h2 style="
  margin-top:45px;
  font-size:24px;
  color:#1d4ed8;
">
  ${authForm.bookingType || 'Booking'} Details
</h2>
  ...
  ${authForm.bookingDetails
    .map(
      (image: string, index: number) => `
        <div style="
          margin-bottom:${index < authForm.bookingDetails.length - 1 ? '25px' : '0'};
        ">
          <img
...
>
  ...
  <div
    ...
    "
  >
    ${authForm.cards?.[0]?.billingAddress || '-'}
  </div>
</div>
