...
export default function CardInformation({ value, onChange, totalAmount }: CardInformationProps) {
  ...
  const syncField = (
    ...
    form.setFieldValue(['card_' + index, field], fieldValue);

    form.validateFields([['card_' + index, field]]).catch(() => {});
  };
  ...
  const removeCard = (index: number) => {
    const updatedCards = value.filter((_, i) => i !== index);
  ...
  const updateCard = <K extends keyof CardInfo>(
    ...
    if (field === 'amount') {
      ...
      if (updatedCards.length > 1 && index !== 0) {
    ...
    syncField(index, field, fieldValue);
  };
  ...
  const paymentLinkRules = [
    ...
    {
      validator: async (_: unknown, link: string) => {
        if (!link) return;

        try {
          new URL(link);
        } catch {
          throw new Error('Please enter a valid payment link');
        }
      },
    },
  ];
  ...
  const cardNumberRules = [
    ...
    {
      validator: async (_: unknown, cardNumber: string) => {
        if (!cardNumber) return;

        const cleanNumber = cardNumber.replace(/\s/g, '');

        if (!/^\d{12,19}$/.test(cleanNumber)) {
          throw new Error('Card number must contain 12-19 digits');
        }
      },
    },
  ];
  ...
  const expiryRules = [
    ...
    {
      validator: async (_: unknown, expiryDate: Dayjs | null) => {
        if (!expiryDate) return;

        if (expiryDate.endOf('month').isBefore(dayjs(), 'month')) {
  ...
  const amountRules = [
    ...
    {
      validator: async (_: unknown, amount: number | null) => {
        ...
        if (amount > totalAmount) {
          throw new Error(`Amount cannot exceed ${totalAmount}`);
        }
      },
    },
  ];
    ...
    <Form form={form} layout="vertical" validateTrigger={['onChange', 'onBlur']}>
      {cards.map((card, index) => {
        ...
        return (
          ...
          >
            <Form.Item label="Card Type" name={[formKey, 'cardType']} rules={cardTypeRules}>
              <Radio.Group
                value={card.cardType}
            ...
            {card.cardType === 'other' ? (
              ...
              <>
                ...
                <Row gutter={16}>
                  ...
                  <Col xs={24} md={8}>
                    <Form.Item label="CVV Number" name={[formKey, 'cvv']} rules={cvvRules}>
                      <Input
                        placeholder="CVV"
                        maxLength={4}
                        inputMode="numeric"
                        value={card.cvv}
                        onChange={(e) =>
                          updateCard(index, 'cvv', e.target.value.replace(/\D/g, ''))
                        }
                      />
                    </Form.Item>
                  </Col>
                    ...
                    >
                      <DatePicker
                        ...
                        disabledDate={(current) =>
                          current.endOf('month').isBefore(dayjs(), 'month')
                        }
            ...
            <Row gutter={16}>
              ...
              <Col xs={24} md={12}>
                <Form.Item label="Amount" name={[formKey, 'amount']} rules={amountRules}>
                  <InputNumber
                    className="w-full"
                    min={0}
                    max={totalAmount}
                    ...
                    value={card.amount}
                    onChange={(amount) =>
                      updateCard(index, 'amount', amount === null ? null : Number(amount))
                    }
                  />
                </Form.Item>
              </Col>
